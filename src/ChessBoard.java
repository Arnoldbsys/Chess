public class ChessBoard {
    ...
    public boolean castling8() {
        if (nowPlayer.equals("White")) {
            if (board[7][7] == null || board[7][3] == null) return false;
            if (board[7][7].getSymbol().equals("R") && board[7][3].getSymbol().equals("K") && // check that King and Rook
                    board[7][4] == null && board[7][5] == null && board[7][6] == null) {              // never moved
                if (board[7][7].getColor().equals("White") && board[7][3].getColor().equals("White") &&
                        board[7][7].check && board[7][3].check &&
                        !new King("White").isUnderAttack(this, 7, 5)) { // check that position not in under attack
                    board[7][3] = null;
                    board[7][5] = new King("White");   // move King
                    board[7][5].check = false;
                    board[7][7] = null;
                    board[7][4] = new Rook("White");   // move Rook
            ...
            if (board[0][7] == null || board[0][3] == null) return false;
            if (board[0][7].getSymbol().equals("R") && board[0][3].getSymbol().equals("K") && // check that King and Rook
                    board[0][4] == null && board[0][5] == null && board[0][6] == null) {              // never moved
                if (board[0][7].getColor().equals("Black") && board[0][9].getColor().equals("Black") &&
                        board[0][7].check && board[0][3].check &&
                        !new King("Black").isUnderAttack(this, 0, 5)) { // check that position not in under attack
                    board[0][3] = null;
                    board[0][5] = new King("Black");   // move King
                    board[0][5].check = false;
                    board[0][7] = null;
                    board[0][4] = new Rook("Black");   // move Rook
